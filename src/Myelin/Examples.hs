...
net = do
    ...
    projection (Static Excitatory (AllToAll (BiasGenerator (Constant 0)) (WeightGenerator (Constant 1.0)))) [input] [a]
    projection (Static Excitatory (AllToAll (BiasGenerator (Constant 0)) (WeightGenerator (Constant 1.0)))) [a] [b]
    projection (Static Excitatory (AllToAll (BiasGenerator (Constant 0)) (WeightGenerator (Constant 1.0)))) [b] [c]
    projection (Static Excitatory (AllToAll (BiasGenerator (Constant 0)) (WeightGenerator (Constant 1.0)))) [c] [a]

net2 :: Monad m => SNN () m
net2 = do
    spike_source <- spikeSourceArray [10 .. 51]
    neurons <- population "neurons" 3 izhikevich Input
    projection (Static Excitatory (OneToOne (BiasGenerator (Constant 0)) (WeightGenerator (Constant 3.0)))) [spike_source] [neurons]
    return ()
